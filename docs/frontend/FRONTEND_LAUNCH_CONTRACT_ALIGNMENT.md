...
3. **Checkout** uses `POST /api/orders/initiate` + Stripe Elements; `return_url` is built through `lib/url/appUrl.ts` so redirects use the runtime app origin with `mosaicbizhub.com` as the production fallback.
...
**Important:** `NEXT_PUBLIC_APP_URL` is used as a configured fallback by `lib/url/appUrl.ts`; Stripe `return_url` prefers the runtime browser origin. Production login should run on `mosaicbizhub.com`; `app.mosaicbizhub.com` is transition-only.
