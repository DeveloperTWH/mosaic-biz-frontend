...
const FoodCard: React.FC<FoodCardProps> = ({
  ...
  const cardContent = (
    <div className="market-card flex h-[420px] flex-col overflow-hidden">
      <div className="market-card-media relative h-[180px] w-full flex-shrink-0">
        <MarketImage src={image} alt={businessName} aspect="video" objectFit="cover" />
        {logo ? (
          <img
            src={logo}
            alt="Business Logo"
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="market-card-footer mt-3 flex min-h-[64px] flex-col items-center justify-center gap-1">
          ...
          {badge ? <TrustBadge tier={badge} size="lg" linkToExplainer /> : <div className="h-16 w-[96px]" />}
        </div>
      </div>
    </div>
  );
