 ...
 const [selectedImage, setSelectedImage] = useState<string | null>(null);
  ...
  const fetchReviews = useCallback(async (listingId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${base}/api/food/${listingId}/reviews?page=1&limit=10`, {
  ...
  const resolvedAverageRating = reviewSummary.averageRating || data.averageRating || 0;
  const resolvedTotalReviews = reviewSummary.totalReviews || data.totalReviews || 0;
  ...
  const handleReviewSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const base = (process.env.NEXT_PUBLIC_API_BASE_URL || "").replace(/\/$/, "");
      const response = await fetch(`${base}/api/food/${foodId}/reviews`, {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
