...
export default function BookingTool({ 
  ...
  const [useBookingTool, setUseBookingTool] = useState(true);

  return (
    <div className="bg-white border border-gray-200 rounded-md p-5">
      <h2 className="text-base font-semibold text-gray-900 mb-4">Booking Tool</h2>
      
      <div className="space-y-4">
        {/* Yes/No Toggle */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Service Booking Tool Required?
          </label>
          <div className="flex items-center gap-6">
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                checked={!useBookingTool}
                onChange={() => {
                  setUseBookingTool(false);
                  onBookingLinkChange('');
                }}
                className="w-4 h-4 text-[#c9a227]"
              />
              <span className="text-sm text-gray-700">No, I will use my own tool.</span>
            </label>
          </div>
        </div>

        {/* Booking Tool Link Input - Only show if Yes is selected */}
        {useBookingTool && (
          <div>
