...
const MapComponent: React.FC<MapComponentProps> = ({ cities }) => {
  ...
  useEffect(() => {
    ...
    d3.json('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json')
      .then((worldData: any) => {
        const geoUnknown = topojson.feature(
          worldData,
        ...
        if (
          ...
          const countries = geoUnknown as FeatureCollection<
            Geometry,
            GeoJsonProperties
          >;

          countries.features = countries.features.filter(
            (feature) => feature.id !== '010'
          );
          ...
          cities.forEach((city) => {
            ...
            if (coords) {
              ...
              cityGroup
                .append('text')
                .attr('x', coords[0] + 8) // slightly more gap
                .attr('y', coords[1] + 5)
                .attr('font-size', '18px') // was 12px → now larger
                .attr('fill', '#000')
